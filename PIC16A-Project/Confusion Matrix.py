...
class classifier:
    ...
    def testing_accuracy(self):
        ...
        #print(self.test[self.y])
        #print(self.clf.predict(self.test[self.X_vars]))

        #CONFUSION MATRIX 
        actual=(self.test[self.y]).to_numpy()
        print(actual, type(actual), actual.shape)
        pred= self.clf.predict(self.test[self.X_vars])
        print(pred, type(pred), pred.shape)
        print(np.isnan(pred).any())
        # confusion_matrix(actual, pred, labels=["Adelie", "Chinstrap", "Gentoo"])
        print(confusion_matrix(actual, pred, labels=[0,1,2]))
        matrix= (confusion_matrix(actual, pred, labels=[0,1,2]))
